...
printMicrostructureParameters();
    ...
    WriteMicrostructureRX(mvUserSettings.mvOutputFolderPath + "/MicroData_Init.tiff");
    else
    {
    MapType = 1;
...
void Microstructure::printMicrostructureParameters() {
    LOG_F(INFO, "I will read microstructure file = %s", mvUserSettings.mvStartFileName);
    LOG_F(INFO, "I will read thermodynamic data from file = %s", mvUserSettings.mvThermodynamicsDataFilename);
    LOG_F(INFO, "I will write all simulation outputs in = %s", mvUserSettings.mvOutputFolderPath);
...
void Microstructure::ReadMicrostructureFile(const std::string& startFileName) {
    ...
    for (int line = 0; line < mvpStateVars->mvN; line++) {
        ...
        // LOG_F(INFO, "read %d and %d", phase, mvpStateVars->GetLatticeIdOfCell(index));
            ...
            OriId = mvOrientations.addOriAndReturnId(mvpStateVars->GetLatticeIdOfCell(index), ea);
...
if (deltaMisorientation < 15.)       *CSLRelationship = 19;
...
void Microstructure::CheckInterfaceAndInterphaseCells() {
    ...
    for (int index = 0; index < mvpStateVars->mvN; index++) {
        ...
        if (IsIPhase)  nInterphases++;
        if (IsIPhase && !mvpStateVars->IsCellInterphase(index)) ABORT_F(" A cell is found with dissimilar (phase) neighbour - YET THIS CELL IS NOT STORED AS PHASE BOUNDARY CELL. The program will exit and check what happened ");
        if (IsIPhase && mvpStateVars->IsCellInterphase(index)) continue;
        ...
        if (IsBoundary)  nBoundaries++;

        if (IsBoundary && !mvpStateVars->IsCellInterface(index)) ABORT_F(" A cell is found with dissimilar (orientation) neighbour - YET THIS CELL IS NOT STORED AS A (SUB)GRAIN BOUNDARY CELL. The program will exit and check what happened ");
...
float Microstructure::SetStateVariablesBasedOnSimAndGiveInfoForMicrostructuralState() {
    ...
    if (!isValidParameter(mvUserSettings.mvRexGGParameterC)) {
       ...
       "Go back to input settings and put C either 0.5, 0.7 or 1., but preferably 0.7",mvUserSettings.mvRexGGParameterC);
    } else {
    ...
    for (int i = 0; i < mvpStateVars->mvN; i++) {
         if (!mvUserSettings.mvReadDislocationDensity) mvpStateVars->mvpRho[i] = 1.e12;
        ...
        if (mvUserSettings.mvIsRexAndGG || mvUserSettings.mvAllowInterfaceMovementDuringPartitioning) {
            ...
            if (mvUserSettings.mvIncludeCSL19FastGrowth) {
                if (mvpStateVars->mvpCSLRelationshipNow == nullptr) mvpStateVars->mvpCSLRelationshipNow = new int[mvpStateVars->mvN];
    ...
    for (int i = 0; i < mvpStateVars->mvN; i++) {
        ...
        for (int nbr = 0; nbr < mvpStateVars->mvNAll; nbr++) {
            ...
            if (mis >= mvUserSettings.mvLowerMisorientationCutOff) {
                ...
                if (mis >= mvUserSettings.mvHAGB) CellIsHAGB = true;
            }
        ...
        if (NNeighs > 0.) {
            ...
            if (CellIsHAGB) mvpStateVars->mvpKAM[i] = mvUserSettings.mvHAGB;
        }
...
double Microstructure::GetMolarVolumeOfCell(int index)
{
   ...
   if (mvpStateVars->IsCellFCC(index))  Vm = mvpTCK->AusteniteMolarVolume();
   if (mvpStateVars->IsCellBCC(index))  Vm = mvpTCK->FerriteMolarVolume();
   if (std::isnan(Vm)) { // Check if Vm is still NaN
...
double Microstructure::GetAreaPerVolumeOfBoundary(int index1, int index2)
{
    ...
    // FIRST CHECK IF WE ARE IN TEST MODE, AND PARTICULARLY FOR THE "WRONG" V-N GRID 
        if (mvUserSettings.mvGridType == 0 && mvpStateVars->mvIs2DSimulation && mvUserSettings.mvTestVonNeumannInSquare) return 1./mvpStateVars->mvDx;
...
float Microstructure::CalcReorientationRatesForFullFieldRexOrGG(int latticeId)
{
    ...
    for (int index : interfaceIndices) {
        if (mvpStateVars->GetLatticeIdOfCell(index) != latticeId) continue;
        iCells++;
        ...
        if (mvUserSettings.mvIncludeCSL19FastGrowth && mvpStateVars->mvpCSLRelationshipNow == nullptr)  mvpStateVars->mvpCSLRelationshipNow = new int[mvpStateVars->mvN];
        ...
        for (int nbr = 0; nbr < NNeighbours; nbr++) {
            ...
            if (mvUserSettings.mvIncludeCSL19FastGrowth && mvpStateVars->mvpCSLRelationshipNow[index] == 0) fDot *= 0.1;
...
int Microstructure::ReOrientCellsForFullFieldRexOrGG(int latticeId) {
    ...
    for (int index : interfaceIndices) {
        if (mvpStateVars->GetLatticeIdOfCell(index) != latticeId) continue;
        isize++;
        if (mvpStateVars->mvpConsumptionRate[index] <= 0.) continue;
        moved++;
        ...
        if (mvpStateVars->mvpConsumedFraction[index] >= 1.) {
            if (mvpStateVars->mvpRX[index] == 0) frx++;
...
LOG_F(INFO, "You have mart diffusivity equal to %e, and you also consider austenite diffusivity being different (but same for all FCC cells) ", mvpTCK->GetCDiffusivityMartensite());
 ...
 if (mvpStateVars->IsCellFCC(index) && mvpStateVars->IsCellFCC(indexNbr) && maxDcFCC < mvpStateVars->mvpDiffusivityC_Ratio[mvpStateVars->mvNNearest * index + nbr]) maxDcFCC=mvpStateVars->mvpDiffusivityC_Ratio[mvpStateVars->mvNNearest * index + nbr];
...
int CSLRelationship;
    ...
    for (int nbr = 0; nbr < mvpStateVars->mvNAll; nbr++){
        indexNbr = NbrList[nbr];
        if (mvpStateVars->GetOriIdOfCell(index) == mvpStateVars->GetOriIdOfCell(indexNbr)) continue;
        float mis = MisorientationBetweenCells(index, indexNbr, true, false, false, &CSLRelationship);
        if (mis > mvUserSettings.mvLowerMisorientationCutOff) return true;
...
int NbrList[26];
    ...
    for (int nbr = 0; nbr < mvpStateVars->mvNAll; nbr++){
        indexNbr = NbrList[nbr];
        if (mvpStateVars->GetLatticeIdOfCell(index) == mvpStateVars->GetLatticeIdOfCell(indexNbr)) continue;
        else return true;
    }
return false;
}
...
void Microstructure::CarbonTemporalOverallQuantitiesForLaterAnalysis()
{
    ...
    Mean_DG_Chem_InFCCCells.push_back(0.);
    Mean_DG_SE_InFCCCells.push_back(0.);
    Mean_DG_Chem_InBCCCells.push_back(0.);
    Mean_DG_SE_InBCCCells.push_back(0.);
...
double MoleFracOfCell = 1.;
    ...
    {
MoleFracOfCell = 1.;
            if (mvUserSettings.mvConsiderConstantIronAtomsPerCell) MoleFracOfCell = (1.-mvUserSettings.mvAverageCarbon)/(1.-(*mvpStateVars->mvpXC)[i]);
