...
/**
 * @brief Constructor for Orientations class.
 * 
 * Initializes the mvpLimits pointer to nullptr.
 */
 ...
 */
int Orientations::addOriAndReturnId(const int ip, const Eigen::Vector3f& ea) {
    mvOrientation.push_back(ea);
    mvCrystalSymmetry.push_back(ip);
...
{
    Eigen::Quaternionf q;
    q = Eigen::Quaternionf(Eigen::AngleAxisf(d2r(ea[0]), Eigen::Vector3f::UnitZ())
        * Eigen::AngleAxisf(d2r(ea[1]), Eigen::Vector3f::UnitX())
        * Eigen::AngleAxisf(d2r(ea[2]), Eigen::Vector3f::UnitZ()));
    return q.normalized();
}
 ...
 */
float Orientations::CalculateMisorientationBetweenTwoOriIds(const int Id1, const int Id2)
{
    float m, w;
 ...
 * @return Misorientation angle in degrees.
 */
float Orientations::CalcMisorientationFromEulerAngles(const Eigen::Vector3f& ea1, const Eigen::Vector3f& ea2)
{
    float m, w;
 ...
 * @return Misorientation angle in degrees.
 */
float Orientations::misorientationQ(const Eigen::Quaternionf q0, const Eigen::Quaternionf q1)
{
    float m, w;
 ...
 * @return Misorientation angle in degrees.
 */
float Orientations::DistanceOfBoundaryFromCSL19a(const Eigen::Vector3f& ea1, const Eigen::Vector3f& ea2)
{
    float m, w;
    for (int ii = 0; ii < 2; ii++) { // switch symmetry must also be applied
        ...
        if (ii < 1) m = fabs(RotatedQ0ToQ1.w());
        else {
